...
const main = async () => {
    ...
    const sphero = await spherov2_js_1.Scanner.findSpheroMiniByName('SM-3F7D');
    if (sphero) {
        await sphero.configureSensorStream();
        await sphero.enableCollisionDetection();
        sphero.on(spherov2_js_1.Event.onCollision, (command) => {
            // tslint:disable-next-line:no-console
            console.log('Sensor Read', command);
        });
        sphero.on(spherov2_js_1.Event.onSensor, (command) => {
