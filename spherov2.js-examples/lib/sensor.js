...
// }
function convertBinaryToFloat(nums, offset, decimals) {
...
const main = async () => {
    function detectVolumeDirection(rotation) {
        ...
        const noramlized = Math.round(((rotation + 180) / 360) * 100) / 100;
        return { volumeUp, scale: noramlized };
    }
    // Example to find a sphereo mini with a certain name
    const sphero = await spherov2_js_lib_1.Scanner.findSpheroMiniByName('SM-3F7D');
    if (sphero) {
        await sphero.configureSensorStream();
        await sphero.enableCollisionDetection();
        sphero.on(spherov2_js_lib_1.Event.onCollision, (command) => {
            // tslint:disable-next-line:no-console
            console.log('Sensor Read', command);
        });
        sphero.on(spherov2_js_lib_1.Event.onSensor, (command) => {
            ...
            const yaw = convertBinaryToFloat(command.payload, 8, 2);
            ...
            // tslint:disable-next-line:no-console
            console.log('Sensor Read', convertBinaryToFloat(command.payload, 0, 2), convertBinaryToFloat(command.payload, 4, 2), convertBinaryToFloat(command.payload, 8, 2), convertBinaryToFloat(command.payload, 12, 2), convertBinaryToFloat(command.payload, 16, 2), convertBinaryToFloat(command.payload, 20, 2));
            // const line = command.payload
            //   .map((i: number) => `${i}`.padStart(3, '0'))
            //   .join(' ');
        });
        while (true) {
            await spherov2_js_lib_1.Utils.wait(1000);
        }
    }
};
main();
